<template>
  <div id="Summary" style="margin: 1% 2%; width: 95%;">
    <v-card flat v-if="this.grades.length < 1">
      <v-row>
      <v-card-title>
        <v-icon class="text-h3" color="black">mdi-numeric-6-box-multiple-outline</v-icon>
      </v-card-title>
      <v-card-title>
        No grades
      </v-card-title>
    ...
    <v-card>
      <v-simple-table v-if="this.$store.state.windowWidth > 1000" class="elevation-0">
        <template v-slot:default>
          ...
          <tbody>
            <tr
              ...
              :key="item.Pozycja"
            >
              <td>{{ item.Przedmiot || ''}}</td>
              <td>{{ item.Srednia || ''}}</td>
              <td>{{ item.ProponowanaOcenaRoczna || ''}}</td>
              <td>{{ item.OcenaRoczna || ''}}</td>
            </tr>
          </tbody>
        </template>
      </v-simple-table>
      <v-simple-table class="elevation-0" v-if="this.$store.state.windowWidth < 1001">
        <template v-slot:default>
          <thead>
            <tr>
              <th class="text-left">
