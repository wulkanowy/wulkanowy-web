<template>
  <div id="login-form" class="fill-height">
    <v-form @submit.prevent="login" class="d-flex fill-height flex-column">
      <v-card-title class="d-flex justify-center text-center">Sign in with the student or parent account</v-card-title>
      <div class="px-5">
        <v-text-field
          outlined
          label="Email"
        ...
        <v-autocomplete
          ...
          :rules="rules.required"
          :items="loginData.selectedRegisterVariantName == 'Fakelog' ? ['powiatwulkanowy', 'adsf'] : symbols"
        />
        <v-select
          outlined
...
export default Vue.extend({
  ...
  computed: {
    host() {
      return RegisterVariants[RegisterVariants.findIndex((item) => item.name === this.loginData.selectedRegisterVariantName)].host;
    },
    ssl() {
      return RegisterVariants[RegisterVariants.findIndex((item) => item.name === this.loginData.selectedRegisterVariantName)].ssl;
    },
  },
